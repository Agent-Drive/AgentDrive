...
class PdfChunker(BaseChunker):
    ...
    def chunk_file(
        ...
        """Parse PDFs via the Document AI online API, splitting into 30-page batches."""
        reader = PdfReader(str(path))
        data = path.read_bytes() if reader.pages and len(reader.pages) <= _MAX_PAGES_PER_BATCH else None
        return self._chunk_from_reader(reader, data, filename, metadata)
