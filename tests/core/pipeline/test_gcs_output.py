...
def _service(mock_client):
    with patch("agentdrive.core.pipeline.storage._get_s3_client", return_value=mock_client):
        with patch("agentdrive.core.pipeline.storage.settings") as mock_settings:
