...
async def test_real_pdf_full_pipeline(db_session: AsyncSession) -> None:
    ...
    with (
        patch(
            "agentdrive.core.pipeline.ingest.StorageService"
        ) as MockStorage,
        patch(
            "agentdrive.core.pipeline.chunking.pdf.PdfChunker._process_batch",
...
async def test_very_large_pdf_multi_batch() -> None:
    ...
    with (
        ...
        patch(
            "agentdrive.core.pipeline.chunking.pdf._processor_name",
