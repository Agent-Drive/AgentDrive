...
from agentdrive.pipeline.queue import (
    _queue,
    _workers,
    enqueue,
    reap_stuck_files,
...
async def test_worker_processes_enqueued_file(db_session_factory, db_session):
    ...
    with patch("agentdrive.pipeline.queue.process_file", mock_process), \
         patch("agentdrive.pipeline.queue.async_session_factory", db_session_factory):
        start_workers(n=1)
        enqueue(file.id)
...
async def test_worker_times_out_and_marks_failed(db_session_factory, db_session):
    ...
    with patch("agentdrive.pipeline.queue.process_file", side_effect=slow_process), \
         patch("agentdrive.pipeline.queue.async_session_factory", db_session_factory), \
...
async def test_worker_handles_unexpected_exception(db_session_factory, db_session):
    ...
    with patch("agentdrive.pipeline.queue.process_file", side_effect=failing_process), \
         patch("agentdrive.pipeline.queue.async_session_factory", db_session_factory):
        start_workers(n=1)
        enqueue(file_id)
...
async def test_reaper_resets_stuck_processing_files(db_session_factory, db_session):
    ...
    with patch("agentdrive.pipeline.queue.async_session_factory", db_session_factory):
